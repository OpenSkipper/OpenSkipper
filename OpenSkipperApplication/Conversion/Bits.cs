...
namespace OpenSkipperApplication
{
    ...
    public class Bits
    {
        ...
        public Bits(bool[] boolBits)
        {
            ...
            {
                throw new ApplicationException("Can't initialize Bits because the input array of boolBits can't be zero length");
            }
        }
        ...
        /// <remarks>
        /// This method uses two's complement method to convert from binary to signed integer number.
        /// </remarks>
        public int ToInt()
        {
            int result = Convert.ToInt32(this.ToUInt());
            int numBits = _bitArray.Length;
            int maxPositiveValue = Convert.ToInt32(Math.Pow(2, numBits - 1) - 1);

            if (result > maxPositiveValue)
            {
                result -= Convert.ToInt32(Math.Pow(2, numBits));
            }
        ...
        public byte[] ToBytes(bool reverse)
        {
            ...
            {
                var curr = _bitArray;
                _bitArray = _bitArray.Reverse().ToArray();
        ...
        public static bool ValidBitString(string data)
        {
        ...
        /// <remarks>
        /// The array is created in a human readable format.  For example,
